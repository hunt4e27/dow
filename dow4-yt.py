...
class YouTubeDownloader:
    ...
    def get_safe_filename(self, url, format_info, video_id):
        ...
        if isinstance(format_info, dict):
            file_type = format_info.get('type', '')
            
            if file_type == 'audio':
                ext = '.mp3'
                # استخدام format للصوت
                quality = format_info.get('format', 'audio').replace(' ', '_')
            elif file_type == 'video':
                ext = '.mp4'
                # استخدام quality للفيديو
                quality = format_info.get('quality', 'video').replace(' ', '_')
            else:
                # في حالة عدم وضوح النوع، نحدد بناءً على المحتوى
                format_text = format_info.get('format', '').lower()
                quality_text = format_info.get('quality', '').lower()
                
                if 'mp3' in format_text or 'audio' in format_text:
                    ext = '.mp3'
                    quality = format_info.get('format', 'audio').replace(' ', '_')
                else:
                    ext = '.mp4'
                    quality = format_info.get('quality', format_info.get('format', 'video')).replace(' ', '_')
        else:
            # في حالة كان format_info نص عادي
            format_str = str(format_info).lower()
            if 'mp3' in format_str or 'audio' in format_str:
                ext = '.mp3'
            else:
                ext = '.mp4'
